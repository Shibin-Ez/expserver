...
void connection_loop_read_handler(void *ptr) {
  ...
  /*set source's ready flag to true*/
...
void connection_source_handler(void *ptr) {
  ...
  int read_n = recv(connection->sock_fd, buff->data, sizeof(buff->data) - 1, 0);
  buff->len = read_n;
...
void connection_sink_handler(void *ptr) {
  ...
  // Write to socket
  int write_n =
      send(connection->sock_fd, buff->data, sink->pipe->buff_list->len, 0);
