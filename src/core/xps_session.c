...
void client_source_handler();
void client_source_close_handler();
void client_sink_handler();
void client_sink_close_handler();
void upstream_source_handler();
void upstream_source_close_handler();
void upstream_sink_handler();
void upstream_sink_close_handler();
void file_sink_handler();
void file_sink_close_handler();
...
xps_session_t *xps_session_create(xps_core_t *core, xps_connection_t *client) {
  ...
  session->to_client_buff = session->from_client_buff = NULL;
  session->upstream_connected = session->upstream_error_res_set = false;
  ...
  if (client->listener->port == 8001) {
    ...
    xps_connection_t *upstream = xps_upstream_create(core, "127.0.0.1", UPSTREAM_PORT);
    if (upstream == NULL) {
...
void xps_session_destroy(xps_session_t *session) {
  ...
  /* destroy client_source, client_sink, upstream_source, upstream_sink and file_sink attached to
   * session */
  xps_pipe_source_destroy(session->client_source);
  xps_pipe_source_destroy(session->upstream_source);
  xps_pipe_sink_destroy(session->client_sink);
...
void client_sink_handler(void *ptr) {
  ...
  xps_pipe_sink_clear(sink, sink->pipe->buff_list->len);
}
